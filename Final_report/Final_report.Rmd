...
<div style="text-align:center">
![](neuron_density1_and_all.png){width=auto height=100%}
</div>
...
<div style="text-align:center">
![](clustered_session1_4.png){width=auto height=100%}
</div>
...
As we can see from the above histograms, in all the session the range of mean firing rate of three clusters have tiny overlap, which means that the cluster labels we get can adequately divide the neurons in all the sessions into three clusters with different mean firing rate range. 
...
The first 3 columns diagnostic plots are from models that based on cluster 1 neurons, cluster 2 neurons and cluster 3 neurons respectively. The last columns is the model based on non-clustered dataset. 
...
As we can see in the ROC curve, AUC of logistic regression is 0.69.
...
<div style="text-align:center">
![](Time_trend_session1_3 .png){width=auto height=100%}
</div>

<div style="text-align:center">
![](Time_trend_session4_5.png){width=auto height=100%}
</div>

As we can see the time trend plots above, the patterns of the first three time trends are quite similar, and the last two session's time trends are also quite similar. According to the background the first 3 session come from the same mouse and the remainder come from another mouse, we guess that this time trend pattern may related to some specific properties of each mouse. Maybe their genders are different or ages are different. However, due to time and information limitation, we can not test this in this paper. Future's work may investigate neurons' response by time  pattern.
